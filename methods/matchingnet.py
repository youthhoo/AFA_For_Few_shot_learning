...
class MatchingNet(MetaTemplate):
  ...
  def set_forward(self, x, is_feature = False, ND = False,epoch = 0):
    if ND:
      z_support, z_query, DC_loss = self.parse_feature(x, is_feature, ND, epoch = epoch)
    else:
      z_support, z_query  = self.parse_feature(x, is_feature, epoch = epoch)
    ...
    if ND:
      return logprobs, DC_loss
    return logprobs

  def set_forward_loss(self, x, ND = False,epoch = 0):
    y_query = torch.from_numpy(np.repeat(range( self.n_way ), self.n_query ))
    y_query = y_query.cuda()
    if ND:
      logprobs, DC_loss = self.set_forward(x, ND = ND, epoch = epoch)
      loss = self.loss_fn(logprobs, y_query)
      return logprobs, loss, DC_loss
      # loss += DC_loss
    else:
      logprobs = self.set_forward(x,epoch = epoch)
      loss = self.loss_fn(logprobs, y_query)
