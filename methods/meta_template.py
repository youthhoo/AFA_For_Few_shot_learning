...
class MetaTemplate(nn.Module):
  ...
  def forward(self, x, ND = False, epoch = 0):
    if ND:
      out, DC_loss = self.feature.forward(x,ND = ND, epoch = epoch)
      return out, DC_loss
    else:
      out = self.feature.forward(x,epoch = epoch)
      return out

  def parse_feature(self, x, is_feature, ND = False, epoch = 0):
    x = x.cuda()
    if is_feature:
      z_all = x
    else:
      ...
      if ND:
        z_all, DC_loss = self.feature.forward(x, ND = ND, epoch = epoch)
      else:
        z_all = self.feature.forward(x, epoch = epoch)
      # z_all = self.feature.forward(x)
      z_all = z_all.view( self.n_way, self.n_support + self.n_query, -1)
    z_support = z_all[:, :self.n_support]
    z_query = z_all[:, self.n_support:]
    if ND:
      return z_support, z_query, DC_loss
