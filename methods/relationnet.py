...
class RelationNet(MetaTemplate):
  ...
  def set_forward(self,x,is_feature = False,ND = False,epoch = 0):
    # get features
    if ND:
      z_support, z_query, DC_loss = self.parse_feature(x, is_feature, ND = ND, epoch = epoch)
    else:
  ...
  def set_forward_loss(self, x,ND = False,epoch = 0):
    ...
    if ND:
      scores,DC_loss = self.set_forward(x,ND = ND, epoch = epoch)
    else:
