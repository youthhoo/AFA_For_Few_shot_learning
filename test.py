...
def evaluate(novel_loader, n_way=5, n_support=5):
    ...
    if 'FT' in params.name or 'ND' in params.name or 'DT' in params.name:
    ...
    for ti, (x, _) in enumerate(novel_loader):  # x:(5, 20, 3, 224, 224)
        ...
        if ti % 100 == 0:
            if ti == 0: 
                print('Task %d : %4.2f%%'%(ti, acc_hundred))
            else:
                acc_hundred/=100
                print('Task %d : %4.2f%%'%(ti, acc_hundred))
            acc_hundred = 0
...
if __name__=='__main__':
    ...
    if params.parts == 'None':
        novel_file = os.path.join('/anonymous', params.dataset,'novel.json')
        
    else:
        novel_file = os.path.join('/anonymous', params.dataset,'product','novel.json')
